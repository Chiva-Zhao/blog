...
//@WebAppConfiguration
@WebMvcTest(controllers = arrayOf(UserController::class, ArticleController::class))
class HttpApiTests(@Autowired val mockMvc: MockMvc) {
    ...
    fun `List Articles`() {
        ...
        whenever(articleRepository.findAllByOrderByAddedAtDesc()).thenReturn(listOf(spring43Article, spring5Article))
        mockMvc.perform(get("/api/article/").accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk)
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
                .andExpect(jsonPath("$.[0].author.login").value(juergen.login))
                .andExpect(jsonPath("$.[0].id").value(spring43Article.id!!))
                .andExpect(jsonPath("$.[1].author.login").value(juergen.login))
                .andExpect(jsonPath("$.[1].id").value(spring5Article.id!!))
    }
